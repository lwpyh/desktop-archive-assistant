...
> ⚠️ 如果子命令真的报"缺少模块/命令未找到"，**只需把报错原样转述给用户**，问是否需要安装——**绝不主动跑安装流程**。默认就当已经装好，直接执行。
...
> 🚫【AI 铁律】**本节仅供人工首次部署参考。处理用户整理请求时，禁止执行本节任何命令（`pip install` / `ollama pull` 等），也禁止把安装当成任务的一步。** 请一律假定环境已就绪，直接执行上方子命令。只有当子命令真的报错缺依赖时，才把报错转述给用户询问，绝不主动安装。
